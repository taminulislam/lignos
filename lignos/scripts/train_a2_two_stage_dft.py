"""A2_chemprop two-stage — DFT-FILLED CACHE variant.

Identical recipe to train_a2_two_stage.py but reads the `_dft.npz` caches
written by scripts/regen_surface_fp_dft_lignoil_a1.py. Those caches have
PointNet-encoded DFT surface_fp filled for 4733/5189 train rows (up from
152/5189 in the original). All other features + targets are unchanged.

Purpose: direct A/B test of "surface_fp from real DFT" vs "surface_fp zero"
on the 0.8401 A2_chemprop baseline.

Expected:
  - If surface_fp is informative: core7 gains +0.005 to +0.015 over 0.8401.
  - If not (head ignores zero-init surface channel anyway): within noise.
  - lignin largely unaffected (Stage-2 head uses physchem, not surface_fp).
"""
...
def load_split(s):
    # Read the DFT-filled variant; fall back to the original if _dft missing.
    p_dft = CACHE / f"cached_{s}_dft.npz"
    p_std = CACHE / f"cached_{s}.npz"
    p = p_dft if p_dft.exists() else p_std
    print(f"[split={s}] loading {p.name}")
...
def main():
    ...
    s1 = summarize("Stage1_A2_chemprop_DFT", stage1_r2s)
    s2 = summarize("Stage2_A2_deep_lignin_physchem_DFT", stage2_r2s)

    print(f"\n{'='*70}\nA2 two-stage SUMMARY (DFT-filled cache)\n{'='*70}")
    ...
    out = V5 / "results" / "a2_two_stage_dft.json"
