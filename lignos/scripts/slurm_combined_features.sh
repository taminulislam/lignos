...
#SBATCH --output=../jobs/combo_ft_%j.out
#SBATCH --error=../jobs/combo_ft_%j.err

module load python/3.10 2>/dev/null || true
source /u/kahmed2/miniconda3/bin/activate mmseg
cd /work/nvme/bgte/kahmed2/Dataset_Chemistry

echo "COMBINED V-JEPA + Supervised ViT Features | $(date)"
...
sup_all=np.load(V5/'data/supervised_vit_features.npz')['features']
sup_tr=sup_all[:152]; sup_te=sup_all[152+32:]
...
print(f'v4 router: avg R2={mv4[\"avg_r2\"]:.4f}')

class PerPropRes(nn.Module):
...
results={}
for name,trf,tef in [('V-JEPA(20D)',vj_tr_p,vj_te_p),('Supervised(20D)',sup_tr_p,sup_te_p),('Combined(40D)',combo_tr,combo_te)]:
    sm=[]
    for seed in range(10):
        set_seed(seed)
        m=PerPropRes(trf.shape[1]).to(device)
    ...
    results[name]=sm
    print(f'\n{name}: avg R2={np.mean(avgs):.4f}+/-{np.std(avgs):.4f}')
    for prop in PROPS:
...
out=V5/'results/combined_features'
