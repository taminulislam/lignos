...
#SBATCH --array=0-467
#SBATCH --output=/work/nvme/bgte/kahmed2/Dataset_Chemistry/lignos/jobs/logs/tier3_dft_%A_%a.out
#SBATCH --error=/work/nvme/bgte/kahmed2/Dataset_Chemistry/lignos/jobs/logs/tier3_dft_%A_%a.err
set -euo pipefail
...
# PCM parser file collision fix (see slurm_dft_new_ils.sh).
cd "$PSI4_SCRATCH"

TASK_LIST=$REPO/data/pipeline/tier3_dft_task_list.txt
TASK_ID=$(sed -n "$((SLURM_ARRAY_TASK_ID + 1))p" "$TASK_LIST")

echo "============================================"
echo "Tier-3 DFT  array=${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
echo "Task: ${TASK_ID}"
echo "Date: $(date)"
echo "============================================"
